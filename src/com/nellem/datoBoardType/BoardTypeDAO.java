package com.nellem.datoBoardType;
...
public class BoardTypeDAO {
	...
	public List<BoardTypeDTO> boardTypeList() {
		...
		try {
			return session.selectList("com.nellem.datoBoardType.selectAll");
		} finally {
			session.close();
		}
	}
	
	public int boardTypeInsert(BoardTypeDTO dto) {
		sqlMapper = getInstance();
		SqlSession session = sqlMapper.openSession();
		
		try {
			return session.insert("com.nellem.datoBoardType.boardTypeInsert", dto);
		} finally {
			session.commit();
			session.close();
		}
